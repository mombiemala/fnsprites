...
- **Changed:** `spriteGuide.js` (the single source for the in-app + static `/sprites#how-sprites-work` guide) now
  leads with the Override season — Cheat Codes / Hack the Lobby codes (plus chests) — and covers the Cheat Master and
  Loot Hacker finishes; Season 3 chest-farming is kept but framed as the archived “Runners” generation.
- **Changed:** the tagline under the logo (app `App.jsx` + static `prerender.mjs`) now reads “Chapter 7 Season 4:
  Override” instead of the stale `v41.30 (Jul 30, 2026)` fallback (the app still shows the live build when detected).
- **Changed:** footer art-credit (app + static, mirrored) and `README.md` brought current — Season 4 uses Epic’s
  official datamined icons; README counts/roster updated (160 released variants; Season 4 is live, not “Upcoming”;
  Bullet→Onigiri; X-Ray, Onigiri, Overshield, Mega Man listed).
- **Why:** a new season means the guide and chrome should describe how Sprites are obtained now, not last season, and
  the whole site should tell one current story.
...
- **Fixed:** regenerated `public/og-image.png` (it still showed the old blue mark) — shared links now
  preview the warm sprite, warm palette and the actual Luckiest Guy wordmark.
- **Why:** after the collectible-card redesign, the old cyan sprite logo was the last thing still on the
  generic community-tracker palette — and it's the first thing seen in the tab, on shares and on the home
  screen. Keeping the sprite everyone recognizes but warming it to the new gradient ties the identity
  together without throwing away recognition.
...
- **Changed:** Gem Aura (`aura` → `vaultedForms: ['gem']` in `src/data/sprites.js`) now
  wears a "Vaulted" badge and isn't counted as currently obtainable. The variant stays `R`
  (owners keep it, so the released-variant count is unchanged), mirroring how Grim Reaper's
  vaulted Gem is handled.
- **Changed:** the v41.30 quests news entry (`news.js`) notes that Gem Aura has since been
  vaulted, so it no longer reads as still-available.
- **Why:** Gem Aura launched through its v41.30 quest, then Epic pulled it again. "Vaulted"
  (not "unreleased") is accurate — unlike Gem King, this one went live, so anyone who earned
  it keeps it while it stops being obtainable for everyone else.
...
- **Changed:** verified the released roster against multiple sources — Air, Seven
  & Batman are correctly live with their full Gold/Gummy/Galaxy/Holofoil lines
  (Batman's specials ramp during Jul 18 Shiny Hours), and Pollo, Vini Jr. & Burnt
  Peanut are correctly Normal-only. Nothing is marked live before it actually is.
- **Added:** News — a teased **Cube Sprite variant** (new special finish expected
  late July, ~Jul 23–30); Cube already shows as a Rumored form on each sprite, so
  nothing flips live until it's confirmed.
- **Added:** News — the leaked **Lucky Locator** item (v41.30, ~Jul 30) that
  reportedly guarantees a Sprite you don't own yet.

*Why:* the tracker's value is being trustworthy about what's actually obtainable,
so "is everything that says live really live?" is worth auditing whenever a wave
of Sprites drops. The leaked Cube/Lucky-Locator dates are deliberately kept out of
the auto-release date-gate (predictions, not firm) — they live in News as
tentative so nothing releases early by mistake.

---

## July 17, 2026 — Shiny Hours (Jul 18) added to the events feed

- **Added:** Shiny Hours event (Sat, Jul 18 · 2 PM & 9 PM ET) — boosted Batman
...
the general DC "Hot Bat Summer" notice — no manual toggling needed.
